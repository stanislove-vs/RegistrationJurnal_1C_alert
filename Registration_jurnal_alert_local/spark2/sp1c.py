...
class JournalEntryFactory: 
    ...
    def get_error_info_type(block): 
        regx = re.compile(r'([EINCU]),"(.*?)",[0-9]', re.DOTALL)
        try:
    ...
    def create_JournalEntry(self, block: str) -> JournalEntry: 
        ...
        # Отправляем маркер некорректного блока журнала
        if None in (timestamp, event_type, message_type, message):
            return False
        # Корректный блок
...
class RegistrationJurnal1C(Jurnal):
    ...
    def __next__(self): 
        ...
        JournalEntry_ =  self.factory.create_JournalEntry(block)
        if JournalEntry_: 
            return JournalEntry_
        else: 
            return False